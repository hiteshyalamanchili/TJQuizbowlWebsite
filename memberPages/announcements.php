...
<head>
    ...
    <title>TJHSST Quizbowl Announcements</title>
...
$ann_query = mysql_query('SELECT * FROM ' . $mySQL_announcements . ' WHERE visibility <= ' . $user_level . ' ORDER BY postdate DESC,posttime DESC LIMIT 0,10;') or die('Couldn\'t make query');

if(mysql_num_rows($ann_query) == 0)
	{
	echo '<strong><p>There are no announcements at this time. Please check back later.</p></strong>';
	}
else
	{
	while(($nextA = mysql_fetch_assoc($ann_query)) !== false)
		{
		if($user_level >= $nextA['visibility'])
			{
            echo "<div class=\"row\">";
            echo "<div class=\"col-lg-12 col-md-12\">";
            echo "<div class=\"panel panel-primary\">";
            echo "<div class=\"panel-heading\">             <h4 class=\"panel-title\">";
            echo $nextA['subject'] . "  (" . $nextA['postdate'] . " " . $nextA['posttime'] . ")";
            if ($nextA['visibility'] == 1)
				{
				echo "\t(Member announcement)";
				}
			if ($nextA['visibility'] == 2)
				{
				echo "\t(Officer announcement)";
				}
            echo "</h4></div>";
			...
			if (isset($_SESSION['level']) && $_SESSION['level'] >= 2)
				{
				echo "<p>Administrator Commands:\n";
				echo "\t\t<a href=\"admin_updateAnnouncementForm.php?announceID=" . $nextA['ID'] . "\">Update</a>\n";
				echo "\t\t<a href=\"admin_deleteAnnouncementExecute.php?announceID=" . $nextA['ID'] . "\">Delete</a>\n";
				echo "\t\t</p>\n";
				}
            echo "</div>";
			echo "<div class=\"panel-footer\">";
			echo "Posted by " . $nextA['author'];
            echo "</div>";
			echo "</div>\n\n";
			echo "</div>" . "\n\n";
            echo "</div>";
